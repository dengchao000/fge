...
*****************************************************************************************/
BOOL CMD5Checksum::GetMD5(const void *pBuf, UINT nLength, WCHAR (&MD5)[33])
{
	//entry invariants
//	IsValidAddress(pBuf,nLength,FALSE);
	...
	MD5Checksum.Update( ( LPBYTE )pBuf, nLength );
	return MD5Checksum.Final( MD5 );
}

BOOL CMD5Checksum::GetMD5(const void *pBuf, UINT nLength, CHAR (&MD5)[33])
{
	//entry invariants
//	IsValidAddress(pBuf,nLength,FALSE);
...
BOOL CMD5Checksum::GetMD5(const void *pBuf, UINT nLength, BYTE (&MD5)[16])
{
	//entry invariants
//	IsValidAddress(pBuf,nLength,FALSE);
...
BOOL CMD5Checksum::GetMD5(Buffer *bufs, UINT cnt, BYTE (&MD5)[16])
{
	//entry invariants
//	IsValidAddress(pBuf,nLength,FALSE);
	...
	for ( UINT i=0; i<cnt; i++ )
		MD5Checksum.Update( ( LPBYTE )bufs[i].pBuf, bufs[i].nLength );
	return MD5Checksum.Final( MD5 );
}

BOOL CMD5Checksum::GetMD5(HANDLE hFile, WCHAR (&MD5)[33])
{
	CMD5Checksum MD5Checksum;		//checksum object	
	DWORD nLength = 0;				//number of bytes read from the file
	const int nBufferSize = 4096;	//checksum the file in blocks of 1024 bytes
	BYTE Buffer[nBufferSize];		//buffer for data read from the file
	while ( true )
	{
		if ( !ReadFile( hFile, Buffer, nBufferSize, &nLength, NULL ) )
			return false;
		if ( nLength == 0 )
			break;
		MD5Checksum.Update( Buffer, nLength );
...
*****************************************************************************************/
BOOL CMD5Checksum::Final( WCHAR (&MD5)[33] )
{
	//Save number of bits
	BYTE Bits[8];
...
//	ASSERT( strMD5.GetLength() == 32 );
	...
	*/
	...
	for ( int i=0; i < nMD5Size; i++) {
		DWORD LB = ( lpszMD5[i] >> 4 );
		DWORD HB = ( lpszMD5[i] & 0xf );
		MD5[i*2] = ( WCHAR )( ( LB > 9 ) ? ( 'A' + ( LB - 10 ) ) : ( '0' + LB ) );
		MD5[i*2+1] = ( WCHAR )( ( HB > 9 ) ? ( 'A' + ( HB - 10 ) ) : ( '0' + HB ) );
	}
	MD5[32] = 0;
	return true;
}

BOOL CMD5Checksum::Final( CHAR (&MD5)[33] )
{
	//Save number of bits
	BYTE Bits[8];
	DWordToByte( Bits, m_nCount, 8 );
...
//	ASSERT( strMD5.GetLength() == 32 );
	...
	*/
	...
	for ( int i=0; i < nMD5Size; i++) {
		DWORD LB = ( lpszMD5[i] >> 4 );
		DWORD HB = ( lpszMD5[i] & 0xf );
		MD5[i*2] = ( CHAR )( ( LB > 9 ) ? ( 'A' + ( LB - 10 ) ) : ( '0' + LB ) );
		MD5[i*2+1] = ( CHAR )( ( HB > 9 ) ? ( 'A' + ( HB - 10 ) ) : ( '0' + HB ) );
	}
	MD5[32] = 0;
	return true;
}

BOOL CMD5Checksum::Final( BYTE (&MD5)[16] )
{
	//Save number of bits
	BYTE Bits[8];
	DWordToByte( Bits, m_nCount, 8 );
	...
	//Store final state in 'lpszMD5'
	const int nMD5Size = 16;
	DWordToByte( MD5, m_lMD5, nMD5Size );
	return true;
}
